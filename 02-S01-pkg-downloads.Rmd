...
Figure \@ref(fig:download-over-time) shows the download trend of all packages on CRAN over time after fixing the unusual spikes. It shows an upward trend over time, and the variance also increases with the increasing of download count, which shows the volatility of the data is increasing.
...
To have a closer look at the weekly pattern, figure \@ref(fig:total-trend) shows the daily download of all CRAN packages from the RStudio mirror with the grey areas highlighting the weekend. 
...
From the previous analysis, we can see that the cumulative download count of R packages shows an increasing trend. If would be perfect equality if every package had the same download count – the last 20% downloaded packages would gain 20% of the total download count or the top 60% downloaded packages would get 60% of the total download count. But we know from experience that this is obviously impossible, so here we introduce Lorenz curve[@lorenz] to show the respective number of packages of different download levels (groups defined by quantiles of download count). In this way, we can figure out how many download counts contributed by different downloaded packages.
...
Figure \@ref(fig:download-over-time) shows cumulative download count against the downloaded groups. It can be seen that most of the download counts comes from the top 10% downloaded packages. At the same time, we can also see that the Gini value is close to 1, which indicates that the download volume among groups is very unbalanced. In fact, the download volume of the top 10% group is extremely distinguished from that of the following groups. It's not hard to understand that this group should contain some packages with high popularity and large quantities of users. For example, if we extract the first 10 packages of this group in Table \@ref(tab:top10), we could find that they are all quite famous and frequently-used ones.
...
```{r lorenz-all, fig.cap="(ref:lorenz-all)"}
ggplot(cran_names_download, aes(total)) +
stat_lorenz() +
