...
`triplet.filter` : reduce the triplets to a single observation. 
...
`size.filter` : filter and removes entries smaller than a package’s binary or source file. 

Here, I have an idea of whether it can be extended to the multi-day download rank percentile of a single package, or the single day download rank percentile of multiple packages, or the multi-day download rank percentile of multiple packages.
