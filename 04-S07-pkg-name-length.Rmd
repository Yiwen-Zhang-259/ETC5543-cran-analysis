...
Coincidentally, we assumed the naming of packages from the same type would also be affected by the initial release time, to a certain extent. So, here, we looked back to the CRAN task view R-packages for constructing comparison among R-packages from the same topic. Figure \@ref(fig:namelth-taskview) shows the name length of CRAN task view R-packages against the initial release date. It is obvious that the length of the R-package name increases with the initial release date, especially for Bayesian R-packages.
...
```{block, type="discovery", echo = TRUE}
**Finding 5**: For all R-packages on CRAN, the average name length tends to generally increase with the inital release date.
```
...
(ref:random-release) The average name length of all R-packages on CRAN each year tends to rise along the time.
