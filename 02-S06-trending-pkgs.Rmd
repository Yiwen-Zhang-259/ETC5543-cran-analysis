...
(ref:trend-updates) The download count increase with the number of updates.

```{r  trend-updates, fig.cap="(ref:trend-updates)}
...
```{block, type="discovery", echo = TRUE}
**Finding 3**: Most trending packages keep updating with the time to keep its activity.
```
