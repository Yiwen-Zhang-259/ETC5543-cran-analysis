...
Here, we still focus on all packages on CRAN to make the conclusion more representative.

```{block, type="discovery", echo = TRUE}
**Finding 1**: The name length of packages has no significant effect on the number of downloads.
```

We can see from Figure \@ref(fig:namelength-plot) that, in general, the influence of name length on download volume is not very obvious. But we could still observe that the number of downloads decreases slightly with the length of the name. And the names of packages with more than 6,000,000 downloads are between 5 and 9 characters long. And the most downloaded is package `r cran_namelth_download$package[1]` whose name length is `r cran_namelth_download$length_name[1]`, with `r cran_namelth_download$total[1]` download counts.
...
```{block, type="discovery", echo = TRUE}
**Finding 5**: For all packages on CRAN, the length of the package name increases with the initial release date as well.
```

Figure \@ref(fig:all-release) shows the name length of all packages from CRAN against the initial release date. The name length of these package increases with the initial release date as well.

(ref:all-release) The name length of all packages from CRAN slightly increase with the initial release date as well.

```{r all-release,fig.cap="(ref:random-release)"}
...
```{block, type="discovery", echo = TRUE}
**Finding 6**: For all packages on CRAN, the average name length tends to generally increase each year.
```

Figure \@ref(fig:avgnamelth-year) shows the average name length of all packages on CRAN each year. It is obvious that the name length of these package generally increases year by year.
