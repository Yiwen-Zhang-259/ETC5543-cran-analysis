...
Figure \@ref(fig:all-updates) shows that the number of downloads increases with the update times. And most R-packages are updated between 1 and 6 times.
...
Also checking Table \@ref(tab:pct-lowupdates), we can know that the percentage of trending R-packages whose updates are less than average is 71 %, which means much more than half of the R-packages do not tend to update very frequently. 
...
It can be seen from Figure \@ref(fig:latestpublish) that most R-packages' latest publish date is after 2021, which indicates that popular R-packages tend to update by time to keep its activity.
