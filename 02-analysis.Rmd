...
pkg_trending_updates <- get_arcvupdate_date(pkg_trending_filter$package) %>%
  group_by(package) %>%
  count(package) %>%
...
total_downloads <- get_total_downloads(packages)

update_counts <- update_counts %>%
  as.data.frame() %>%
  cbind(total_downloads)

return(update_counts)

}

get_updates_downloads(pkg_trending_filter$package)
