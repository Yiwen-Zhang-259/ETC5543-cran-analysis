...
total_download <- cran_downloads(from = "2020-08-01", to = "2021-04-01")


total_download$weekend <- weekdays(total_download$date) %in% c("Saturday", "Sunday")

ggplot(total_download,aes( x = date, y = count)) +
  ...
  geom_smooth(se = F) +
   labs( x = "Date",
    ...
    subtitle = "from 2020-08-01 to 2021-04-01") +
  theme_minimal()  +
...
for (i in c(1:length(task_topic_all$topic))) {
   ...
   pkg_taskview_total <- rbind(df_total,df)
...
Let's have a look at the bayesian packages. It can be seen from Figure \@ref(fig:bayesian) that similarly, the earlier packages is released, the more dowloads it will have. And most of the packages are from 2007 to 2012.
...
Table \@ref(tab:pct-lowupdates) shows the top 30 trending packages these days. I will compare the number of updates of them with their package scores.
...
pkg_trending_daily_update %>%
  mutate(update_perday = release_days/`number of updates`) %>%
...
pkg_random <- sample(x,500,replace = TRUE) %>%
  as.data.frame()
...
pkg_random1 %>%
  ...
  labs( y = 'Total download counts',
    ...
    subtitle = "for 100 random packages on CRAN") +
  theme_minimal()  +
  theme(panel.grid.major = element_blank()) +
  theme(
  axis.text=element_text(size=10),
...
`total average name length`  <- mean(db_namelth$length_name)
`pkg_trending_total` <- length(pkg_random$package)
...
pkg_random1 <- cbind(pkg_random,`average name length`)


nrow(pkg_random1$package)
```
...
sample.mean <- mean(pkg_trending2$length_name)
sample.n <- length(pkg_trending2$length_name)
sample.sd <- sd(pkg_trending2$length_name)
...
```{r ahlpha-downloads}
 alphabetic_downloads <- get_downloads_alphabetic(pkg_random1)
