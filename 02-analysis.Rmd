...
topic_pkg <- topic_pkg %>%
  dplyr::rename(package = packagelist.name) %>%
  filter_list() %>%
...
doe_pkgs <- ctv:::.get_pkgs_from_ctv_or_repos("Econometrics", 
...
`total average name length` <- mean(pkg_trending2$length_name)
...
trending_top30_namelth <- avg_namelth(pkg_trending_30) %>%
  mutate(group = "top 30")

trending_tail30_namelth <- avg_namelth(pkg_trending_tail30) %>%
  mutate(group = "last 30")

trending_all_namelth <- avg_namelth(pkg_trending) %>%
  mutate(group = "all")

compare_trending_namelth <- rbind(trending_top30_namelth,trending_tail30_namelth,trending_all_namelth)
```
