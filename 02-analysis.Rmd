...
1. `fable` vs `forecaset`

It can be seen from Figure \@ref(fig:fcst-fbl) that although package `fable` is older than `forecast`, its download count is much less than that of `forecast`.

```{r fcst-fbl, fig.align = 'center', fig.cap="The older package 'fable' has less download count than that of package 'forecast'."}
...
fcst_fble <- rbind(forecast,fable) %>%
  group_by(package) 

fcst_fble <- as.data.frame(fcst_fble)

release_date <- c("2021-03-11","2021-02-03")

fcst_fble <- cbind(fcst_fble,release_date) 
