...
commits <- c(get_commits(fddm),get_commits(gRc),get_commits(webfakes),get_commits(tree.interpreter),get_commits(geospark),get_commits(spatstat.core),get_commits(AzureStor),get_commits(AzureRMR),get_commits(spatstat.linnet),get_commits(js),get_commits(clickstream),get_commits(spatstat.sparse))


# to find the top 15 trending packages
pkg_trending <- cran_trending() %>%
  head(15)

# exclude the package 'proxy', 'uplift' and 'spatstat.geom' that only have read-only mirror of the CRAN github repo
pkg_trending_filter <- pkg_trending %>%
...
```{r commits-pattern, fig.align = 'center', fig.cap=" The package with most updates is 'proxy" while the most trending one is 'fddm'.}
pkg_trending_commits %>%
  ...
  labs( y = 'Score',
    ...
    annotate("text",y= 850,x= 275,label="'fddm' with the highest score has 319 commits",color="red") +
  theme_minimal()  +
  theme(panel.grid.major = element_blank()) +
  theme(
  axis.text=element_text(size=10),
...
pkg_trending %>%
  kable(caption = "Top 15 trending package") %>%
  kable_styling(bootstrap_options = c("hover", "striped")) 

# find the updates of top 15 trending packages manually
updates <- c("5","13","2","2","2","2","28","2","3","17","16","3","5","15","4") 

pkg_trending_updates <- cbind(pkg_trending,updates)
```
...
```{r}
latest_publish <- c("2021-03-05","2021-03-16","2021-02-17","2020-01-30","2021-04-03","2021-04-06","","","","","","","","","")
```
