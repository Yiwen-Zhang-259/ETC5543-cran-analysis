...
pkg_rstusio <- lapply(pkg_rstusio,list)
...
pkg_secondary <- lapply(pkg_secondary,list)
```

```{r}
# filter the packages developed by top 20 prolific maintainers
pkg_20maintainer <- db %>%
  filter(grepl('Hadley Wickham|Yihui Xie|Dirk Eddelbuettel|Jeroen Ooms|Achim Zeileis|Scott Chamberlain|Gabor Csardi|Jeroen Ooms|ORPHANED|Thomas J. Leeper|Bob Rudis|Henrik Bengtsson|Kurt Hornik|Oliver Keyes|Martin Maechler|Richard Cotton|Robin K. S. Hankin|Simon Urbanek|Kirill Muller|Torsten Hothorn|Paul Gilbert', Author)) %>%
  select(Package)

pkg_20maintainer <- lapply(pkg_20maintainer,list)
```
