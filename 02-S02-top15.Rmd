...
For 2014, `ncdf` ranks first, which is used to provide an interface to netCDF format data. For 2015, the first downloaded is `XLConnectJars` on JAVA dependency. For 2016, the first one is `reports` for standardising the output of R. For 2017, `lava.tobit` aims to help with estimation and simulation of latent variable models. For 2018, the ranking-first `ReportsRs` is for creating Microsoft Word and Powerpoint documents. And for 2019, `CALIBERrfimpute` is used to impute missing values in analysis datasets using full conditional specifications. As for 2020, `SparkR` is similar to `dplyr` but for large datasets. At last, for 2021, `heatmap.plus` is an extension of `heatmap()` function.
